...
## 후속 (2026-08-17): 제목 변경 시 파일 분열 — 부분 수정 + 알려진 제약

### 문제

`File.resolvePaperPath`가 `title`을 포함해 저장 경로를 결정하는데, `refreshContent`가
arXiv 개정판을 재조회해 `paper.title`을 덮어쓰면 다음 저장 시 계산되는 경로가 바뀐다.
`writePaperAt`은 "새 경로에 파일이 있는가"로만 기존 여부를 판단하므로 옛 파일을 못 찾고
새 파일을 만들어 `createdAt`/임베딩/`extra`/`collectedApis` 이력이 고아가 됐다(실제 재현).

### 고친 것

`refreshAllBody`가 `Refresh()` 호출 전 `paper.title`을 스냅샷해두고, 호출 후 값이
달라졌으면 `File.renamePaperFiles(oldSnapshot, paper)`로 옛 경로의 `.json`/`.md`를
새 경로로 옮긴 뒤 저장한다. **`refreshContent`가 제목을 바꾸는 이 한 경로만** 감지한다
— 원인(무엇이 왜 바뀌었는지)을 코드가 이미 알고 있는 유일한 지점이라 국지적 수정으로
충분했다.

### 알려진 제약 — 사용자가 파일을 직접 리네임하는 경우는 못 잡는다

사용자가 파일 탐색기에서 `.json`/`.md`의 **파일명만** 직접 바꾸면(내용의 `title` 필드는
안 건드림), 위 로직은 이 상황을 감지하지 못한다:

1. `readAllPapers`가 `.json` 내용을 읽음 → `paper.title`은 여전히 원래 값(내용은 안
   바뀌었으므로)
2. `Refresh()`가 arXiv에서 재조회 → 실제 개정이 없었으면 **같은 제목**이 돌아옴
3. `titleBefore === paper.title` → "제목 안 바뀜"으로 판단 → rename 로직이 아예 안 돎
4. `resolvePaperPath(paper)`가 원래(내용 기준) 제목의 경로를 계산 → 사용자가 옮겨간
   자리엔 파일이 없음 → "새 논문"으로 보고 새로 생성 → 사용자가 리네임한 파일은 고아로
   남음

`재수집`(recent/backfill) 경로도 동일한 이유로 같은 증상을 보인다 — arXiv에서 새로
파싱한 `Paper`는 항상 현재 실제 제목을 가져오므로, 그 경로에 파일이 없으면 새로 만든다.

**근본 원인**: 이 시스템은 논문의 신원을 `sourceId`가 아니라 "지금 계산되는 경로"로
판단한다. 경로가 어긋나는 원인이 코드가 아는 것(arXiv 개정)이면 보정 가능하지만,
사용자의 직접 조작처럼 코드가 감지할 단서가 없는 원인이면 보정할 수 없다.

**일반적으로 고치려면** `sourceId -> 실제 경로` 인덱스가 필요하다 — 경로 계산이 실패하면
코퍼스 전체를 `sourceId`로 스캔하거나(수천 편 규모에서 매 저장마다 비용 큼), 쓰기/삭제/
이름변경마다 유지보수해야 하는 별도 인덱스 파일을 도입해야 한다(인덱스 자체가 또
어긋날 수 있는 새 상태를 만듦). 두 방법 다 이번 수정 범위보다 훨씬 무거워, **의도적으로
범위 밖으로 남겨두고 알려진 제약으로 문서화**하기로 결정함(2026-08-17).
