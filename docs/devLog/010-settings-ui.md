...
### 후속 수정 — "추려진" 개수가 신규 발견량을 뜻하지 않던 문제

사용자 리포트: 대기열의 "추려진" 개수가 실제와 안 맞고, 페이지 크기(100편)씩 뛴다.

원인은 두 가지가 섞여 있었다. (1) arXiv 한 페이지가 최대 100편(`ArxivAPI.PAGE_SIZE`)이라
`'all'`이 페이지 단위로 불려 100씩 뛰는 것 자체는 정상. (2) 진짜 문제는 "최근 논문
수집"이 색인 지연에 대응하려고 `recentRescanWindowMs`(4일, `API.ts`)만큼 항상 겹치는
구간을 다시 훑는다는 점 — 이미 저장된 논문도 매번 이 청크에 다시 걸리는데,
`CollectFoundMiddleware`는 "청크에 도착한 논문 수"만 셀 뿐 이미 아는 논문인지 구분하지
않아 그대로 더했다. 그래서 "추려진 300편"이 떠도 실제 신규는 0~소수일 수 있었다.

해결: `CollectFoundMiddleware`가 청크의 각 논문을 `File.readStoredPaper()`로 확인해
저장소에 없던(신규) 논문만 `apiNewFound`로 따로 센다. `CollectAndSave.ts`는 여전히
안 건드린다 — `File`은 정적 유틸리티라 미들웨어에서 바로 호출할 수 있고,
`prefillFromStore`가 같은 목적의 조회를 이미 하고 있어 완전히 새로운 패턴도 아니다.
대신 페이지당(최대 100회) 읽기가 한 번 더 늘어난다 — 코퍼스 전체를 훑는 비용은 아니라
감수했다. 대기열 보조 줄은 "신규 N편 · 재확인 M편 · 처리 K편"으로 바뀌었다.
