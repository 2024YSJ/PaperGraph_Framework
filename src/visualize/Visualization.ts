...
export class Visualization {
	...
	// z 방향으로 축 선을 긋고, 연도 경계마다 라벨 스프라이트를 배치한다.
	private drawTimeAxis(scene: Scene, graph: GraphData): void {
		const line = new Line(
			new BufferGeometry().setFromPoints([new Vector3(0, 0, 0), new Vector3(0, 0, Z_LENGTH)]),
		...
		for (let year = startYear; year <= endYear; year += 1) {
			...
			sprite.position.set(0, 0, ((t - min) / span) * Z_LENGTH);
			scene.add(sprite);
		}
	}
